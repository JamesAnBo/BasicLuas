...
--Midcast sets (Magic Attack Bonus / Cure Potency / Skill+ / etc.)
	...
	Phalanx = {},
    Stoneskin = {},
    SIR = {--Spell Inertuption Rate Down. Used during your summoning midcast.
	},
	Nuke = {},
	Enfeebling = {},
    Drain = {},
...
local Settings = {
	...
	MacroBook = 15; -- The macro book you want for this job. Otherwise set to false.
...
local function HandlePetAction(PetAction)
	if (blinclude.SmnSkill:contains(PetAction.Name)) then
        gFunc.EquipSet(sets.SmnSkill);
	elseif (blinclude.SmnMagical:contains(PetAction.Name)) then
        gFunc.EquipSet(sets.SmnMagical);
    elseif (blinclude.SmnHybrid:contains(PetAction.Name)) then
        gFunc.EquipSet(sets.SmnHybrid);
	elseif (blinclude.SmnHealing:contains(PetAction.Name)) then
        gFunc.EquipSet(sets.SmnHealing);
    elseif (blinclude.SmnEnfeebling:contains(PetAction.Name)) then
...
profile.OnLoad = function()
	...
	SetLockstyle:once(2);
	SetMacros:once(3);
    blconfig.settings.RegenGearHPP = 50;
    blconfig.settings.RefreshGearMPP = 60;
    blconfig.settings.PetDTGearHPP = 30;
end
...
profile.HandleDefault = function()
	...
	if (pet ~= nil) then
		...
		if (petElement ~= nil) and (blinclude.GetCycle('Weapon') == 'Default') then
			gFunc.Equip('main', blsets.ElementalStaffTable[petElement]);
		end
		...
		if (pet.Status == 'Engaged') then
			if (player.Status == 'Engaged') then
				gFunc.EquipSet(sets.Both_Engaged);
			else
...
profile.HandleAbility = function()
    ...
    if (blinclude.GetToggle('JACancel') == true) and  (canJA == false) then
		...
		if (ability.Name == 'Release') or (ability.Name == 'Assault') or (ability.Name == 'Retreat') then 
			return
		end
