...
local Settings = {
	...
	MacroBook = 7; -- The macro book you want for this job. Otherwise set to false.
...
profile.HandleDefault = function()
    ...
    if (player.Status == 'Engaged') then
        ...
        if (blinclude.GetCycle('TankSet') ~= 'None') then
			gFunc.EquipSet('Tank_' .. blinclude.GetCycle('TankSet')) end
	...
	if (party:GetMemberHPPercent(0) >= 85) then
...
profile.HandleAbility = function()
    ...
    if (blinclude.GetToggle('JACancel') == true) and  (canJA == false) then
		...
		if string.match(ability.Name, 'Sentinel') then
...
profile.HandlePrecast = function()
    ...
    elseif (spell.Skill == 'Healing Magic') then
		...
		if (target.Name == me) then
			gFunc.EquipSet(sets.Self_Cure_Precast);
		end
    end
...
profile.HandleMidcast = function()
	...
	elseif (spell.Skill == 'Divine Magic') then
		if string.match(spell.Name, 'Flash') then
			gFunc.EquipSet(sets.Flash);
		elseif string.match(spell.Name, 'Enlight') then
			gFunc.EquipSet(sets.Enlight);
		elseif (string.contains(spell.Name, 'Banish')) or (string.contains(spell.Name, 'Holy')) then
			if (spell.Element == weather.WeatherElement) or (spell.Element == weather.DayElement) then
				obiLib:Evaluate(0.1);
			end
		end
    ...
    if (blinclude.GetToggle('SIR') == true) then
        gFunc.EquipSet(sets.SIR);
    end
